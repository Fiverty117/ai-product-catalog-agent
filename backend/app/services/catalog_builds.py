...
def create_catalog_build(
    ...
    *,
    ...
    """Create snapshot + v2 render Job + build owner in one DB transaction.
...
def get_catalog_build(session: Session, build_id: uuid.UUID) -> CatalogBuildRead:
    ...
    job = build.job
    if job is None or job.job_type != CATALOG_RENDER_JOB_TYPE_V2:
        raise CatalogBuildIntegrityError("Catalog build render Job is invalid")
    try:
        payload = CatalogRenderJobPayloadV2.model_validate(job.payload)
...
def resolve_catalog_build_artifact_pdf(
    ...
    *,
    ...
    if (
        ...
        or run.job is None
        or run.job.job_type != CATALOG_RENDER_JOB_TYPE_V2
