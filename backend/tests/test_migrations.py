...
def test_0018_preserves_0017_catalog_history_and_adds_copy_constraints(
    ...
    assert {"product_copy_runs", "product_copy_reviews", "product_copy_manual_revisions", "product_identity_edits"} <= set(
    ...
    with engine.connect() as connection:
        ...
        assert connection.scalar(text("SELECT count(*) FROM product_identity_edits")) == 0
