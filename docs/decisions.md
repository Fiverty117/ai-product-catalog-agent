...
## ADR-029 — Historical duplication is a current-state Builder template
**Decision:** The read-only duplication plan uses frozen Snapshot Product IDs and frozen presentation configuration only to initialize the existing in-memory Builder. It resolves current Product readiness and Publisher identity by stable IDs, with explicit warnings for unavailable or ambiguous choices. A new Build is created only by the ordinary Builder create service and gets a fresh Snapshot. `CatalogBuild.source_build_id` is optional provenance and participates in explicit request idempotency, but never render identity. New render Jobs may carry optional Builder-choice provenance for palette overrides, logo visibility and Publisher-contact overrides; this metadata is excluded from render identity and never consumed by the renderer. Older Jobs without that provenance default safely with warnings instead of inferring intent from resolved values.

**Why:** Historical commerce must remain immutable while reusing editorial intent. An in-memory draft avoids a second persistence lifecycle, and a narrow nullable lineage reference records origin without changing catalog content or PDF behavior.
